...
// We need to initialize the data, for example, initialize all entries to 0 to avoid the memory leak.
  for (int i = 0; i < 10; i++) {
    x[i] = 0;
  }
...
//
  for ( i = 0; i < 10; i++ )
  {
    x[i] = 2 * x[i]; // This reports error by VALGRIND MEMCHECK if some x[i] is uninitialized, because of involving computing uninitialized data.
  }
//
//  Print X.
//
...
// This reports error by VALGRIND MEMCHECK if some x[i] is uninitialized, because of involving printing uninitialized data.
