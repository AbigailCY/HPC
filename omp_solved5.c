...
// change the order of locks here. For example, in the original file, when section1 locks a, section2 locks b, then the next step for section1 is to lock b, whereas next step for section2 is to lock a, which causes a deadlock.
    #pragma omp section
      {
